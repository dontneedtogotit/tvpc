...
# 5. Create HTPC user (added 'input' for remotes; 'netdev' for Wi-Fi resilience)
HTPC_USER="htpc"
if ! id "$HTPC_USER" &>/dev/null; then
  useradd -m -G video,render,audio,plugdev,input,netdev -s /bin/bash "$HTPC_USER"
...
pulse.cmd = [
  { cmd = "set-card-profile" args = "alsa_card.pci-0000_00_1f.3 output:hdmi-stereo-extra" },
  { cmd = "set-default-sink"  args = "alsa_output.pci-0000_00_1f.3.hdmi-stereo-extra" }
]
EOF

# 8. Load i915 with GuC/HuC for Broadwell HW decode
cat >/etc/modules-load.d/i915.conf <<'EOF'
i915
EOF
if ! grep -q "i915.enable_guc" /etc/default/grub; then
  sed -i 's/^GRUB_CMDLINE_LINUX_DEFAULT="\(.*\)"/GRUB_CMDLINE_LINUX_DEFAULT="\1 i915.enable_guc=2 intel_iommu=igfx_off quiet splash"/' /etc/default/grub
  update-grub
fi
...
# 18. Run post-install extras (HW decode verification, NUC tuneables)
...
echo "  powertop --auto-tune            # re-apply power tuning after USB/wifi settle"